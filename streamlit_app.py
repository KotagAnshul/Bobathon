...
# ─────────────────────────────────────────────────────────────
# IBM CARBON DARK THEME — pixel-matching dashboard.html
# ─────────────────────────────────────────────────────────────
st.markdown("""
<style>
  /* ══════════════════════════════════════════
     ROOT — force #161616 everywhere in modern Streamlit
  ══════════════════════════════════════════ */
  ...
  /* Wipe any lingering white from Streamlit's default injected styles */
  .stApp { color: #F4F4F4 !important; }
  ...
  /* Sidebar toggle when sidebar is collapsed — keep it visible inside our bar */
  ...
  :root {
    ...
    --border:#525252; --text:#F4F4F4; --text-muted:#A8A8A8; --text-disabled:#6F6F6F;
    --green:#42BE65; --yellow:#F1C21B; --red:#FA4D56;
    font-family: -apple-system, "Segoe UI", system-ui, sans-serif;
  }

  /* ── Global text & bg ── */
  html, body { background: #161616 !important; color: #F4F4F4 !important; }
  p, span, div, label, li, td, th { color: #F4F4F4; }
  .stMarkdown, .stMarkdown p { color: #F4F4F4 !important; }
  ...
  [data-testid="stSidebar"] {
    ...
    top: 48px !important;          /* sit below our header */
    z-index: 999 !important;       /* below our header (99999) */
  }
  [data-testid="stSidebar"] > div:first-child { padding: 0 !important; }
  [data-testid="stSidebar"] * { color: #F4F4F4 !important; }

  /* Hide Streamlit's default sidebar padding wrapper */
  ...
  /* ── Nav items — st.button styled as nav rows ── */
  ...
  [data-testid="stSidebar"] .nav-active .stButton > button {
    background: #0F62FE1A !important;
    color: #F4F4F4 !important;
    border-left: 2px solid #0F62FE !important;
  ...
  /* ── Dividers ── */
  hr { border: none !important; border-top: 1px solid #525252 !important; margin: 16px 0 !important; }
  ...
  /* ── News action column buttons — ghost style, compact ── */
  ...
  div[data-testid="column"]:last-child .stButton > button:hover {
    background: #393939 !important; color: #F4F4F4 !important;
  }

  /* ── Refresh button — smaller, ghost ── */
  div[data-testid="column"]:has(button[kind="secondary"]) .stButton > button,
  .refresh-btn .stButton > button {
    background: transparent !important; border: 1px solid #525252 !important;
    color: #A8A8A8 !important; font-weight: 400 !important; font-size: 12px !important;
  ...
  input, textarea, select {
    background: #393939 !important; color: #F4F4F4 !important;
    border: 1px solid #525252 !important; border-radius: 0 !important; font-size: 13px !important;
  }
  input:focus, textarea:focus { outline: 2px solid #0F62FE !important; outline-offset: 0 !important; }
  [data-testid="stTextInput"] > div > div { background: #393939 !important; border-radius: 0 !important; }

  /* ── Selectbox ── */
  [data-testid="stSelectbox"] > div > div {
    background: #393939 !important; border: 1px solid #525252 !important; border-radius: 0 !important;
  }
  [data-testid="stSelectbox"] svg { fill: #A8A8A8 !important; }

  /* ── Expanders ── */
  [data-testid="stExpander"] {
    background: #262626 !important; border: 1px solid #525252 !important; border-radius: 0 !important;
  }
  [data-testid="stExpander"] summary { color: #F4F4F4 !important; font-weight: 600 !important; }
  [data-testid="stExpander"] > div { background: #1e1e1e !important; }

  /* ── DataFrames ── */
  [data-testid="stDataFrame"] th {
    background: #393939 !important; color: #A8A8A8 !important;
  ...
  [data-testid="stChatMessage"] {
    background: #262626 !important; border: 1px solid #525252 !important;
    border-radius: 0 !important;
  }
  [data-testid="stChatInput"] { background: #262626 !important; border-top: 1px solid #525252 !important; }
  [data-testid="stChatInput"] textarea { background: #393939 !important; }
  ...
  .tag-opportunity { background:#071908; color:#42BE65; border:1px solid #198038; }
  .tag-risk        { background:#2D0709; color:#FA4D56; border:1px solid #DA1E28; }
  .tag-competitor  { background:#1C0F30; color:#BE95FF; border:1px solid #8A3FFC; }
  .tag-exec        { background:#302400; color:#F1C21B; border:1px solid #684E00; }
  .tag-blue        { background:#001141; color:#78A9FF; border:1px solid #0043CE; }
  .tag-strong      { background:#071908; color:#42BE65; border:1px solid #198038; }
  .tag-moderate    { background:#302400; color:#F1C21B; border:1px solid #684E00; }
  .tag-cold        { background:#2D0709; color:#FA4D56; border:1px solid #DA1E28; }
  .tag-advantaged  { background:#071908; color:#42BE65; border:1px solid #198038; }
  .tag-even        { background:#302400; color:#F1C21B; border:1px solid #684E00; }
  .tag-at-risk     { background:#2D0709; color:#FA4D56; border:1px solid #DA1E28; }

  /* Cards */
  .ibm-card { background:#262626; border:1px solid #525252; padding:20px; margin-bottom:16px; }
  .card-title { font-size:11px; font-weight:600; color:#A8A8A8; text-transform:uppercase;
    letter-spacing:.5px; margin-bottom:12px; }

  /* News items — action buttons live INSIDE the card HTML, not as st.button */
  .news-item { background:#262626; border:1px solid #525252; border-left:3px solid #525252;
  ...
  .news-item.tag-border-exec { border-left-color:#684E00; }
  .news-item.tag-border-opp  { border-left-color:#198038; }
  .news-item.tag-border-comp { border-left-color:#8A3FFC; }
  .news-body  { flex:1; min-width:0; }
  .news-meta  { font-size:12px; color:#A8A8A8; margin-top:4px; }
  .news-summary { font-size:13px; color:#A8A8A8; margin-top:6px; line-height:1.5; }
  .news-actions { display:flex; flex-direction:column; gap:6px; flex-shrink:0; padding-top:2px; }
  .btn-sm {
    font-size:11px; padding:4px 10px; border:1px solid #525252;
    background:transparent; color:#A8A8A8; cursor:pointer; white-space:nowrap;
    font-family:-apple-system,"Segoe UI",sans-serif; text-align:center;
    display:block; width:96px;
  }
  .btn-sm:hover { background:#393939; color:#F4F4F4; }
  .btn-sm.flagged { background:#302400; color:#F1C21B; border-color:#684E00; }
  ...
  .stat-card   { background:#262626; border:1px solid #525252; padding:16px; }
  .stat-label  { font-size:11px; color:#A8A8A8; text-transform:uppercase; letter-spacing:.4px; margin-bottom:6px; }
  .stat-value  { font-size:22px; font-weight:700; color:#F4F4F4; }
  .stat-sub    { font-size:12px; color:#A8A8A8; margin-top:2px; }
  .text-green  { color:#42BE65 !important; }
  .text-red    { color:#FA4D56 !important; }
  .text-yellow { color:#F1C21B !important; }
  .text-blue   { color:#78A9FF !important; }

  /* Progress bars */
  .progress-bar  { height:6px; background:#393939; width:100%; margin-top:5px; }
  .progress-fill { height:100%; background:#0F62FE; }
  .progress-fill.high { background:#FA4D56; }
  .progress-fill.warn { background:#F1C21B; }

  /* Chips */
  .chip     { display:inline-block; background:#393939; border:1px solid #525252;
    color:#A8A8A8; font-size:11px; padding:2px 8px; border-radius:12px; margin:2px; }
  .chip.ibm { background:#001141; color:#78A9FF; border-color:#0043CE; }
  .chip-row { display:flex; flex-wrap:wrap; gap:4px; margin-top:6px; }

  /* Meeting cards */
  .meeting-card   { background:#262626; border:1px solid #525252; padding:18px; margin-bottom:14px; }
  .meeting-header { display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:10px; }
  .meeting-title  { font-size:15px; font-weight:600; color:#F4F4F4; }
  .meeting-meta   { font-size:12px; color:#A8A8A8; margin-top:3px; }
  .prep-badge     { font-size:11px; font-weight:600; padding:3px 10px; border-radius:2px; flex-shrink:0; }
  .prep-complete  { background:#071908; color:#42BE65; border:1px solid #198038; }
  .prep-needs     { background:#302400; color:#F1C21B; border:1px solid #684E00; }
  .prep-no        { background:#2D0709; color:#FA4D56; border:1px solid #DA1E28; }
  .attendee-label { font-size:12px; color:#A8A8A8; margin-top:8px; margin-bottom:3px; }

  /* LinkedIn prospect cards */
  .li-card          { background:#262626; border:1px solid #525252; padding:16px 18px; margin-bottom:12px; }
  ...
  .li-card-name     { font-size:14px; font-weight:600; color:#F4F4F4; }
  .li-card-role     { font-size:12px; color:#A8A8A8; margin-top:2px; }
  .li-card-location { font-size:12px; color:#A8A8A8; margin-top:2px; }
  .li-card-reason   { font-size:12px; color:#78A9FF; margin-bottom:10px; line-height:1.5; }
  .li-reason-label  { font-size:11px; color:#A8A8A8; text-transform:uppercase; letter-spacing:.4px; display:block; margin-bottom:2px; }
  .li-card-link     { font-size:12px; color:#78A9FF; text-decoration:none; border:1px solid #0043CE; padding:4px 12px; display:inline-block; }
  .li-card-badge    { flex-shrink:0; margin-left:10px; }

  /* Tables */
  .ibm-table { width:100%; border-collapse:collapse; font-size:13px; }
  .ibm-table th { text-align:left; font-size:11px; font-weight:600; color:#A8A8A8;
    text-transform:uppercase; letter-spacing:.4px; padding:8px 12px; border-bottom:1px solid #525252; }
  .ibm-table td { padding:10px 12px; border-bottom:1px solid #333; vertical-align:top; color:#F4F4F4; }
  .ibm-table tr:last-child td { border-bottom:none; }
  .ibm-table tr:hover td { background:#2a2a2a; }
  .ibm-table td.muted { color:#A8A8A8; }

  /* Bullet list */
  .bullet-list { list-style:none; padding:0; margin:0; }
  .bullet-list li { padding:8px 0; border-bottom:1px solid #333; font-size:13px;
    display:flex; gap:10px; align-items:flex-start; }
  .bullet-list li:last-child { border-bottom:none; }
  .bullet-dot { width:6px; height:6px; border-radius:50%; background:#0F62FE;
    margin-top:6px; flex-shrink:0; }
  .bullet-dot.green  { background:#42BE65; }
  .bullet-dot.yellow { background:#F1C21B; }
  .bullet-dot.blue   { background:#0F62FE; }

  /* Alert banners */
  .renewal-alert { background:#2D1800; border:1px solid #684E00; color:#F1C21B;
    font-size:12px; padding:6px 12px; margin-bottom:8px; }
  .upsell-alert  { background:#001141; border:1px solid #0043CE; color:#78A9FF;
    font-size:12px; padding:6px 12px; margin-bottom:8px; }
  ...
  /* Section subtitle */
  .section-sub { font-size:13px; color:#A8A8A8; margin-top:2px; margin-bottom:20px; }

  /* Chat bubbles */
  .bubble-bob  { background:#393939; border:1px solid #525252; padding:10px 14px;
    font-size:13px; line-height:1.5; color:#F4F4F4; margin-bottom:8px; }
  .bubble-user { background:#0F62FE; padding:10px 14px; font-size:13px;
    line-height:1.5; color:#fff; margin-bottom:8px; text-align:right; }
  .msg-name      { font-size:11px; color:#A8A8A8; margin-bottom:3px; }
  .msg-name-user { font-size:11px; color:#A8A8A8; margin-bottom:3px; text-align:right; }
</style>
...
<style>
  ...
  /* ── Card wrapper: centre everything ── */
  .login-shell {
    display: flex;
  ...
  /* Remove Streamlit's wrapper border on inputs inside login */
  .login-box [data-testid="stTextInput"] > div {
    background: transparent !important;
    border: none !important;
    border-radius: 0 !important;
  }
  .login-box [data-testid="stTextInput"] > div > div {
    background: transparent !important;
    border: none !important;
  }
  ...
  .login-box .stButton > button:hover { background: #0353E9 !important; }

  /* ── Checkbox ── */
  .login-box [data-testid="stCheckbox"] label {
    color: #161616 !important;
    font-size: 14px !important;
  }
  .login-box [data-testid="stCheckbox"] p { color: #161616 !important; }
...
""", unsafe_allow_html=True)
    ...
    # All inputs are in the centre column; CSS gives that column the card look.
    _lg, _lc, _lr = st.columns([1, 2, 1])
    with _lc:
        # Full card header: IBM blue top-strip + heading + subtitle
        st.markdown(
            '<div class="login-box">'
            '  <div class="login-ibm-bar"></div>'
            '  <div class="login-body">'
            '    <p class="login-heading">Sign in with your<br>w3id password</p>'
            '    <p class="login-sub">Use your w3id and password</p>'
            '  </div>'
            '</div>',
            unsafe_allow_html=True,
        )
        # Inputs section — same card box, border continues from the header
        st.markdown(
            '<div class="login-box login-inputs">',
            unsafe_allow_html=True,
        )
...
with st.sidebar:
    # ── Header: IBM logo + rep info + account tag ──────────────
    st.markdown(f"""
<div style="padding:20px 16px 12px;border-bottom:1px solid #525252;">
  <div style="font-size:20px;font-weight:700;color:#0F62FE;letter-spacing:-0.5px;margin-bottom:2px;">IBM</div>
  <div style="font-size:12px;color:#6F6F6F;letter-spacing:0.3px;">Sales Intelligence Hub</div>
</div>
<div style="padding:14px 16px 12px;border-bottom:1px solid #333;">
  <div style="font-size:13px;font-weight:600;color:#F4F4F4;margin-bottom:2px;">{rep}</div>
  <div style="font-size:12px;color:#6F6F6F;">IBM Global Account Executive</div>
  <div style="margin-top:8px;">
    <span style="display:inline-block;font-size:11px;font-weight:600;padding:3px 10px;
      background:#001141;color:#78A9FF;border:1px solid #0043CE;
      text-transform:uppercase;letter-spacing:.4px;">{account}</span>
  </div>
</div>
""", unsafe_allow_html=True)
    ...
    # ── Footer: date + logout ─────────────────────────────────
    st.markdown(
        f'<div style="border-top:1px solid #525252;padding:12px 16px 8px;'
        f'font-size:12px;color:#6F6F6F;margin-top:8px;">'
...
<style>
  ...
  .ibm-topbar {{
    ...
    background: #000; border-bottom: 1px solid #393939;
  ...
  /* ── Account switcher ──
     The selectbox with key="account_switcher" renders inside:
       div[data-testid="stVerticalBlock"]          (column)
         > div[data-testid="element-container"]    ← has data-key
           > div[data-testid="stSelectbox"]
     We pin the column's element-container to the top-right. */

  /* The element-container wrapping the selectbox */
  ...
  div[data-testid="element-container"][data-key="account_switcher"] [data-baseweb="select"] > div {{
    background: #262626 !important; border: 1px solid #393939 !important;
    border-radius: 0 !important; min-height: 32px !important; padding: 0 8px !important;
  }}
  div[data-testid="element-container"][data-key="account_switcher"] [data-baseweb="select"] span {{
    color: #78A9FF !important; font-weight: 700 !important; font-size: 13px !important;
  }}
  div[data-testid="element-container"][data-key="account_switcher"] svg {{
    fill: #78A9FF !important;
  }}

  /* "Account:" label — pinned left of the dropdown */
  .hdr-acct-label {{
    position: fixed !important; top: 14px !important; right: 293px !important;
    font-size: 13px !important; color: #A8A8A8 !important;
    font-family: -apple-system,"Segoe UI",system-ui,sans-serif !important;
    white-space: nowrap !important; z-index: 99999 !important;
  }}
  /* Rep name — pinned to the far right */
  .hdr-rep {{
    position: fixed !important; top: 14px !important; right: 16px !important;
    font-size: 13px !important; color: #F4F4F4 !important;
    font-family: -apple-system,"Segoe UI",system-ui,sans-serif !important;
    white-space: nowrap !important; z-index: 99999 !important;
  }}
</style>
...
<div class="ibm-topbar">
  <div style="display:flex;align-items:center;gap:0;">
    <span style="font-size:18px;font-weight:700;color:#0F62FE;letter-spacing:-0.5px;">IBM</span>
    <div style="width:1px;height:20px;background:#393939;margin:0 14px;"></div>
    <span style="font-size:14px;color:#F4F4F4;">Sales Intelligence Hub</span>
  </div>
</div>

<span class="hdr-acct-label">Account:</span>
<span class="hdr-rep">{rep}</span>
""", unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
    ...
    with hdr_col:
        if api_key_set:
            st.markdown(
                f'<div style="font-size:12px;color:#42BE65;margin-bottom:12px;">'
                f'⚡ <strong>Live</strong> — Tavily web search · {account} · {profile["industry"]}</div>',
                unsafe_allow_html=True
            )
        else:
            st.markdown(
                '<div style="font-size:12px;color:#A8A8A8;margin-bottom:12px;">'
    ...
    for a in (display_alerts or [])[:3]:
        ...
        src_link = (
            f'<a href="{a["url"]}" target="_blank" style="color:#78A9FF;font-size:12px;text-decoration:none;">'
            f'Read more →</a>' if a.get("url") else ""
        )
        alert_rows_html += (
            f'<div style="margin-bottom:12px;">'
            f'  <div style="font-size:13px;font-weight:600;color:#F4F4F4;margin-bottom:3px;">{a["title"]}</div>'
            f'  <div style="font-size:12px;color:#A8A8A8;">{a["date"]} · {a["source"]}</div>'
            f'  <div style="font-size:12px;color:#A8A8A8;margin-top:4px;line-height:1.5;">{preview}</div>'
            f'  <div style="margin-top:4px;">{src_link}</div>'
            f'</div><hr style="margin:10px 0;border-color:#333;">'
        )
    if not alert_rows_html:
        alert_rows_html = '<div style="font-size:13px;color:#6F6F6F;">Live industry alerts appear here when Tavily is enabled.</div>'
    ...
    for _item in (display_news or [])[:4]:
        ...
        _dot_color = (
            "#FA4D56" if "risk" in _tl.lower() or "competitor" in _tl.lower()
            else "#F1C21B" if "executive" in _tl.lower()
            else "#42BE65"
        )
        _headline_rows += (
            f'<div style="display:flex;gap:10px;align-items:flex-start;'
            f'padding:8px 0;border-bottom:1px solid #333;">'
            f'  <span style="width:7px;height:7px;border-radius:50%;background:{_dot_color};'
            f'             flex-shrink:0;margin-top:5px;"></span>'
            f'  <div style="min-width:0;">'
            f'    <div style="font-size:12px;color:#F4F4F4;line-height:1.4;'
            f'               white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">'
            f'      {_item["title"]}</div>'
            f'    <div style="font-size:11px;color:#6F6F6F;margin-top:2px;">'
...
<div class="grid-2" style="margin-bottom:24px;">
  <div class="ibm-card">
    ...
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:14px;">
      <span style="font-size:32px;font-weight:700;color:#F4F4F4;">{len(display_news)}</span>
      <span style="font-size:13px;color:#A8A8A8;">news items today</span>
    </div>
    ...
    <div style="border-top:1px solid #333;padding-top:4px;">
      <div style="font-size:11px;font-weight:600;color:#6F6F6F;text-transform:uppercase;
...
""", unsafe_allow_html=True)
    ...
    for item in display_news:
        ...
        src_link_html = (
            f'<a href="{url}" target="_blank" style="color:#78A9FF;text-decoration:none;font-size:12px;">'
            f'{src_display}</a>' if url else f'<span style="font-size:12px;color:#A8A8A8;">{src_display}</span>'
        )
        read_more_html = (
            f'<a href="{url}" target="_blank" style="color:#78A9FF;font-size:12px;text-decoration:none;'
            f'border:1px solid #0043CE;padding:3px 10px;background:#001141;">'
        ...
        if bullets:
            bullets_html = (
                '<div style="margin:10px 0 8px;padding:10px 12px;background:#1e1e1e;'
                ...
                + "".join(
                    ...
                    f'  <span style="font-size:13px;color:#C6C6C6;line-height:1.5;">{b}</span>'
                    f'</div>'
                    for b in bullets
                )
                + "</div>"
        ...
        preview_html = (
            f'<div style="font-size:12px;color:#A8A8A8;line-height:1.5;margin-top:8px;'
            f'padding-top:8px;border-top:1px solid #333;">{preview}</div>'
            if preview else ""
        )

        # Full card HTML
        st.markdown(
            f'<div style="{dismissed}background:#262626;border:1px solid #525252;border-left:3px solid transparent;margin-bottom:4px;padding:16px 18px;" class="{border}">'
            ...
            f'{src_link_html} &nbsp;<span style="color:#525252;font-size:12px;">·</span>&nbsp;'
            f'<span style="font-size:12px;color:#A8A8A8;">{item["date"]}</span>'
            f'</div>'
            f'<div style="font-size:14px;font-weight:600;color:#F4F4F4;line-height:1.4;margin-bottom:2px;">{item["title"]}</div>'
            f'{bullets_html}'
            f'{preview_html}'
        ...
        # Action buttons — full-width row, proper sizing, below the card
        b1, b2, b3, _spacer = st.columns([2, 2, 2, 6])
        with b1:
    ...
    with col2:
        ...
        for t in tickets:
            ...
            tickets_html += (
                ...
                f'  <div style="font-size:12px;color:#A8A8A8;margin-top:2px;">{t["status"]}</div>'
                f'</div><hr style="margin:8px 0;">'
            )
        st.markdown(f"""
<div class="ibm-card">
...
""", unsafe_allow_html=True)
    ...
    for comp in competitors:
        ...
        comp_rows += (
            ...
            f'  <td style="color:#A8A8A8;">{comp["products"]}</td>'
...
""", unsafe_allow_html=True)
        ...
        if is_expanded:
            ...
            note_rows = "".join(
                f'<tr>'
                f'<td style="white-space:nowrap;color:#A8A8A8;font-size:11px;padding:8px 10px 8px 0;vertical-align:top;min-width:80px;">{n["date"]}</td>'
                f'<td style="white-space:nowrap;color:{"#42BE65" if n.get("new") else "#78A9FF"};font-size:11px;padding:8px 10px 8px 0;vertical-align:top;min-width:110px;">{n["author"]}{"&nbsp;✓" if n.get("new") else ""}</td>'
                f'<td style="font-size:13px;line-height:1.55;padding:8px 0;color:#F4F4F4;">{n["text"]}</td>'
                f'</tr>'
                for n in notes
            )
            st.markdown(f"""
<div style="background:#1a1a1a;border:1px solid #525252;border-left:3px solid #0F62FE;padding:18px 20px;margin-bottom:4px;">
  <div style="font-size:11px;font-weight:600;color:#A8A8A8;text-transform:uppercase;letter-spacing:0.6px;margin-bottom:10px;padding-bottom:6px;border-bottom:1px solid #393939;">Saved Notes</div>
...
""", unsafe_allow_html=True)
            ...
            # ── Bob AI Summary ────────────────────────────────────
            ai_paras = "".join(
                f'<p style="margin-bottom:10px;font-size:13px;line-height:1.65;color:#F4F4F4;">'
                f'<strong style="color:#78A9FF;">{label}:</strong> {body}'
                f'</p>'
                for label, body in ai_summ
            )
            st.markdown(f"""
<div style="background:#001141;border:1px solid #0043CE;padding:18px 20px;margin-bottom:16px;">
  <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;">
    <span style="font-size:12px;font-weight:600;color:#78A9FF;text-transform:uppercase;letter-spacing:0.5px;">Bob AI Summary</span>
  ...
  {ai_paras}
  <div style="font-size:11px;color:#78A9FF;margin-top:4px;opacity:0.7;">⚡ Generated by sales_hub_orchestrator_agent · meetings_relationships_agent</div>
</div>
""", unsafe_allow_html=True)
    ...
    for r in relationship_map:
        ...
        border_color = "#42BE65" if "Strong" in el else "#F1C21B" if "Moderate" in el else "#525252"

        last_meeting_html = (
            f'<div style="margin-top:10px;padding-top:10px;border-top:1px solid #393939;">'
            f'<div style="font-size:11px;font-weight:600;color:#A8A8A8;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:6px;">Last Meeting</div>'
            f'<div style="font-size:12px;color:#A8A8A8;margin-bottom:3px;">{h["date"]} &nbsp;·&nbsp; {h["title"]}</div>'
            f'<div style="font-size:13px;color:#F4F4F4;line-height:1.5;">{h["outcome"]}</div>'
            f'</div>'
        ) if h else (
            f'<div style="margin-top:10px;padding-top:10px;border-top:1px solid #393939;">'
            f'<div style="font-size:12px;color:#6F6F6F;font-style:italic;">No meeting history found.</div>'
            f'</div>'
        )

        contact_cards += f"""
<div style="background:#262626;border:1px solid #525252;border-left:3px solid {border_color};padding:16px 18px;margin-bottom:10px;">
  <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:8px;">
    <div>
      <div style="font-size:15px;font-weight:600;color:#F4F4F4;">{r["contact"]}</div>
      <div style="font-size:12px;color:#A8A8A8;margin-top:2px;">{r["title"]}</div>
    </div>
    <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
      <span class="tag {ec}" style="font-size:11px;">{el}</span>
      <span style="font-size:11px;color:#A8A8A8;">IBM Owner: <span style="color:#78A9FF;">{r["ibm_owner"]}</span></span>
      <span style="font-size:11px;color:#A8A8A8;">Last contact: {r["last"]}</span>
    </div>
  </div>
  {last_meeting_html}
</div>"""
    ...
    if st.session_state.linkedin_error:
        ...
        st.markdown(
            f'<div style="font-size:13px;color:#FA4D56;padding:12px 0;">{msg}</div>',
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            '<div style="font-size:11px;color:#6F6F6F;padding:8px 0 4px;">'
        ...
        st.markdown(
            '<div style="font-size:13px;color:#A8A8A8;padding:12px 0;">'
...
<p class="section-sub">Pre-built IBM Sales AI Agents — expand any card to run the agent for {account}.</p>
""", unsafe_allow_html=True)
    ...
    with st.expander("🎯 Lead Identification"):
        ...
        for lead in leads:
            sc = lead["score"]
            sc_cls = "text-green" if sc >= 85 else "text-yellow" if sc >= 70 else "text-red"
            score_rows += (
                f'<tr><td><strong>{lead["name"]}</strong></td>'
                f'<td>{lead["title"]}</td>'
                f'<td>{lead["dept"]}</td>'
                f'<td class="{sc_cls}" style="font-weight:600;">{sc}</td>'
                f'<td class="muted">{lead["reason"]}</td></tr>'
            )
        st.markdown(f"""
<table class="ibm-table">
  <thead><tr><th>Contact</th><th>Title</th><th>Department</th><th>Score</th><th>Reason</th></tr></thead>
...
""", unsafe_allow_html=True)
    ...
    with st.expander("✉️ Client Outreach Draft"):
        st.markdown(f"""
<div style="font-size:12px;color:#A8A8A8;margin-bottom:8px;">
  <strong>To:</strong> {profile['client_name']} &lt;{profile['client_email']}&gt;<br>
  <strong>Subject:</strong> Continuing Our Conversation — IBM watsonx &amp; EU AI Act Compliance
</div>
<div style="font-size:13px;line-height:1.7;background:#1e1e1e;border:1px solid #525252;padding:16px;">
Dear {profile['client_name']},<br><br>
I hope this note finds you well. Following our recent discussions, I wanted to reach out personally regarding new IBM watsonx capabilities directly relevant to your EU AI Act compliance timeline.<br><br>
IBM watsonx.governance now includes dedicated tools for model explainability, audit trails, and bias detection — built specifically for regulated industries ahead of the Q3 enforcement deadline.<br><br>
Would you be available for a 30-minute call the week of June 23rd?<br><br>
Warm regards,<br>{rep} · IBM GAE<br><br>
<span style="font-size:11px;color:#A8A8A8;">⚡ Powered by IBM Client Outreach AI Agent (Salesforce + Seismic)</span>
</div>
""", unsafe_allow_html=True)
    ...
    with st.expander("🔍 Client Search (D&B)"):
        st.markdown(f"""
<table class="ibm-table">
  <tr><td class="muted">Revenue</td><td>{profile["revenue"]}</td></tr>
  <tr><td class="muted">Ticker</td><td>{profile["ticker"]}</td></tr>
  <tr><td class="muted">Cloud</td><td>Azure · AWS · GCP · IBM Cloud</td></tr>
  <tr><td class="muted">AI Platforms</td><td>IBM watsonx · Azure OpenAI · Vertex AI</td></tr>
  <tr><td class="muted">CRM / ERP</td><td>Salesforce · SAP S/4HANA</td></tr>
  <tr><td class="muted">Employees</td><td>{profile["employees"]}</td></tr>
  <tr><td class="muted">HQ</td><td>{profile["hq"]}</td></tr>
</table>
""", unsafe_allow_html=True)
    ...
    with st.expander("📊 CRM Snapshot (Salesforce)"):
        ...
        opp_rows = "".join(
            f'<tr><td>{o["name"]}</td>'
            f'<td><span class="tag tag-blue">{o["stage"]}</span></td>'
            f'<td style="font-weight:600;">{o["amount"]}</td>'
            f'<td>{o["close"]}</td>'
            f'<td class="muted">{o["forecast"]}</td></tr>'
            for o in crm_opps
        )
        st.markdown(f"""
<table class="ibm-table">
  <thead><tr><th>Opportunity</th><th>Stage</th><th>Amount</th><th>Close</th><th>Forecast</th></tr></thead>
  <tbody>{opp_rows}</tbody>
</table>
<div style="margin-top:12px;font-size:14px;font-weight:600;">Total Pipeline: <span class="text-green">${total_pipeline:,.0f}</span></div>
""", unsafe_allow_html=True)
    ...
    with st.expander("📚 Seismic Content"):
        sc_items = "".join(
            ...
            f'<span style="font-size:12px;color:#A8A8A8;">{s["type"]} · Relevance: {s["relevance"]}</span></div></li>'
            for s in seismic_content
        )
        st.markdown(f'<ul class="bullet-list">{sc_items}</ul>', unsafe_allow_html=True)

    with st.expander("🧠 Sales Research Brief"):
        st.markdown(f"""
<div style="font-size:13px;line-height:1.6;margin-bottom:12px;">
  <strong>{account}</strong> is a major global services and consulting organization. IBM remains a strategic
  enterprise AI and data platform partner with active watsonx, Cloud Pak for Data, and Sterling deployments.
</div>
<div class="card-title">Top Signals</div>
<ul class="bullet-list">
  <li><span class="bullet-dot yellow"></span>Executive sponsor {profile["client_name"]} is active — re-engage this quarter.</li>
  <li><span class="bullet-dot"></span>Governance and renewal timing remain the main near-term priorities.</li>
  <li><span class="bullet-dot"></span>Competitive pressure from hyperscalers across AI platform decisions.</li>
  <li><span class="bullet-dot green"></span>Expansion potential across watsonx, data, and observability workloads.</li>
</ul>
<div style="margin-top:12px;" class="card-title">D&amp;B Profile</div>
<table class="ibm-table">
  <tr><td class="muted">Revenue</td><td>{profile["revenue"]}</td></tr>
  <tr><td class="muted">Ticker / Credit</td><td>{profile["ticker"]}</td></tr>
  <tr><td class="muted">Employees</td><td>{profile["employees"]}</td></tr>
  <tr><td class="muted">HQ</td><td>{profile["hq"]}</td></tr>
</table>
""", unsafe_allow_html=True)
...
<p class="section-sub">Powered by IBM watsonx Orchestrate · <code>sales_hub_orchestrator_agent</code> · Click any question to fire it into Bob</p>
""", unsafe_allow_html=True)

    # Chat history rendered with IBM bubble style
    for msg in st.session_state.chat_history:
        if msg["role"] == "bob":
            st.markdown(
                f'<div class="msg-name">💼 Bob &nbsp;<span style="color:#42BE65;font-size:10px;">● Live</span></div>'
                f'<div class="bubble-bob">{msg["content"]}</div>',
                unsafe_allow_html=True
            )
        else:
            st.markdown(
                f'<div class="msg-name-user">You</div>'
    ...
    # Sample questions
    st.markdown('<div class="card-title" style="margin-bottom:12px;">Sample Questions — click to ask Bob</div>', unsafe_allow_html=True)
    cols = st.columns(3)
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .ph-title {{ font-size: 14px; font-weight: 700; color: #F4F4F4; }}
  .ph-sub {{ font-size: 11px; color: #6F6F6F; }}
  .ph-right {{ display: flex; align-items: center; gap: 10px; }}
  .ph-live {{ font-size: 11px; color: #42BE65; letter-spacing: .2px; }}
  .ph-close {{
    background: none; border: none; color: #6F6F6F;
    cursor: pointer; font-size: 20px; line-height: 1;
    padding: 4px; display: flex; align-items: center;
  }}
  .ph-close:hover {{ color: #F4F4F4; }}
  ...
  #msgs::-webkit-scrollbar-track {{ background: #161616; }}
  #msgs::-webkit-scrollbar-thumb {{ background: #525252; border-radius: 2px; }}

  .msg {{ display: flex; flex-direction: column; max-width: 86%; gap: 3px; }}
  .msg-bob {{ align-self: flex-start; }}
  .msg-user {{ align-self: flex-end; }}
  .msg-label {{
    font-size: 10px; color: #6F6F6F; font-weight: 600;
  ...
  .msg-bob .bubble {{
    background: #262626; border: 1px solid #393939; color: #F4F4F4;
  }}
  .msg-user .bubble {{
  ...
  .dot {{
    width: 6px; height: 6px; border-radius: 50%; background: #6F6F6F;
  ...
  .sugg-label {{
    font-size: 10px; color: #6F6F6F; text-transform: uppercase;
  ...
  .sugg-btn:hover {{ border-color: #0F62FE; color: #F4F4F4; background: #222; }}
