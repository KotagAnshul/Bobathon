...
<div class="login-card">
  <div class="login-logo">IBM</div>
  <h2 style="font-size:20px;font-weight:600;color:#F4F4F4;margin-bottom:4px;">Sales Intelligence Hub</h2>
  <p style="font-size:13px;color:#A8A8A8;margin-bottom:28px;">Powered by watsonx Orchestrate — Bob</p>
</div>
""", unsafe_allow_html=True)
        rep = st.text_input("YOUR NAME", value="Sarah Mitchell", key="login_rep")
        account = st.selectbox("SELECT ACCOUNT", ["Accenture", "Deloitte", "PwC", "EY", "KPMG"], key="login_account")
...
# ── Fixed top header bar — pure st.markdown so position:fixed is relative
#    to the real browser viewport with zero layout gap.
st.markdown(f"""
<div style="
...
">
  ...
  <div style="display:flex;align-items:center;gap:14px;">
    <div style="display:flex;align-items:center;gap:8px;background:#262626;
                border:1px solid #393939;padding:5px 14px;font-size:13px;">
      <span style="color:#A8A8A8;">Account:</span>
      <span style="color:#0F62FE;font-weight:700;">{account}</span>
    </div>
    <span style="font-size:13px;color:#F4F4F4;">{rep}</span>
  </div>
</div>
""", unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
        note_val = st.text_input(
            ...
            key=f"note_{m['id']}", label_visibility="collapsed"
        )
        if st.button(f"💾 Save Note — {m['id']}", key=f"savenote_{m['id']}"):
            if note_val.strip():
                st.toast(f"✅ Note saved to CRM for {m['id']} · {rep} · {date.today().strftime('%b %d, %Y')}", icon="✅")
            else:
                st.toast("⚠️ Please enter a note before saving.", icon="⚠️")

    st.markdown('<hr class="divider">', unsafe_allow_html=True)
    st.markdown('<div class="card-title" style="margin-bottom:12px;">Meeting History (Last 5)</div>', unsafe_allow_html=True)

    hist_rows = ""
    for h in meeting_history:
        hist_rows += (
            f'<tr><td>{h["date"]}</td><td>{h["title"]}</td>'
            f'<td><span class="chip">{h["attendees"]}</span></td>'
            f'<td class="muted">{h["outcome"]}</td></tr>'
        )
    st.markdown(f"""
<div class="ibm-card">
  <table class="ibm-table">
    <thead><tr><th>Date</th><th>Meeting</th><th>Key Attendees</th><th>Outcome</th></tr></thead>
    <tbody>{hist_rows}</tbody>
  </table>
</div>
""", unsafe_allow_html=True)

    st.markdown('<hr class="divider"><div class="card-title" style="margin-bottom:12px;">Relationship Map</div>', unsafe_allow_html=True)
    ...
    for r in relationship_map:
        ...
        rel_rows += (
            f'<tr><td><strong>{r["contact"]}</strong></td><td>{r["title"]}</td>'
            f'<td>{r["ibm_owner"]}</td>'
            f'<td><span class="tag {ec}">{el}</span></td>'
            f'<td>{r["last"]}</td></tr>'
        )
    st.markdown(f"""
<div class="ibm-card">
  <table class="ibm-table">
    <thead><tr><th>Contact</th><th>Title</th><th>IBM Owner</th><th>Engagement</th><th>Last Interaction</th></tr></thead>
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  #fab {{
    ...
    box-shadow: 0 4px 16px rgba(15,98,254,0.45);
    transition: background 0.15s, transform 0.15s;
  }}
  #fab:hover {{ background: #0353E9; transform: scale(1.07); }}

  #panel {{
    position: fixed; bottom: 88px; right: 24px;
    width: 360px; height: 500px;
    background: #262626; border: 1px solid #525252;
  ...
  .ph {{
    padding: 12px 14px; border-bottom: 1px solid #525252;
    display: flex; justify-content: space-between; align-items: center;
    flex-shrink: 0;
  }}
  .ph-title {{ font-size: 14px; font-weight: 600; color: #F4F4F4; }}
  .ph-sub {{ font-size: 11px; color: #6F6F6F; margin-top: 1px; }}
  .ph-right {{ display: flex; align-items: center; gap: 10px; }}
  .ph-live {{ font-size: 11px; color: #42BE65; }}
  .ph-close {{
  ...
  #msgs::-webkit-scrollbar-track {{ background: #1e1e1e; }}
  #msgs::-webkit-scrollbar-thumb {{ background: #525252; }}

  .msg {{ max-width: 88%; }}
  .msg-bob {{ align-self: flex-start; }}
  .msg-user {{ align-self: flex-end; }}
  .msg-name {{ font-size: 10px; color: #6F6F6F; margin-bottom: 3px; }}
  .msg-user .msg-name {{ text-align: right; }}
  .bubble {{
    padding: 9px 13px; font-size: 12px; line-height: 1.55;
    white-space: pre-wrap; word-break: break-word;
  }}
  .msg-bob .bubble {{ background: #393939; border: 1px solid #525252; color: #F4F4F4; }}
  .msg-user .bubble {{ background: #0F62FE; color: #fff; }}

  .typing {{ display: flex; gap: 4px; align-items: center; padding: 10px 13px; }}
  .dot {{
    width: 6px; height: 6px; border-radius: 50%; background: #A8A8A8;
  ...
  .sugg-btn {{
    background: transparent; border: 1px solid #525252; color: #A8A8A8;
    font-size: 11px; padding: 3px 9px; cursor: pointer; font-family: inherit;
  }}
  .sugg-btn:hover {{ border-color: #0F62FE; color: #F4F4F4; }}

  .inp-row {{
    padding: 10px 12px; border-top: 1px solid #525252;
    display: flex; gap: 8px; flex-shrink: 0;
  }}
  #inp {{
    flex: 1; background: #393939; border: 1px solid #525252; color: #F4F4F4;
    padding: 8px 10px; font-size: 12px; font-family: inherit; outline: none;
  }}
  #inp:focus {{ border-color: #0F62FE; outline: 2px solid #0F62FE; outline-offset: 0; }}
  #inp::placeholder {{ color: #6F6F6F; }}
  #send-btn {{
...
<button id="fab" onclick="togglePanel()" title="Ask Bob">💬</button>

<div id="panel">
  <div class="ph">
    <div>
      <div class="ph-title">Ask Bob</div>
      <div class="ph-sub">sales_hub_orchestrator_agent</div>
    </div>
    <div class="ph-right">
  ...
  <div id="msgs"></div>
  <div class="sugg">
    <button class="sugg-btn" onclick="suggest(this)">Exec summary</button>
    <button class="sugg-btn" onclick="suggest(this)">Competitor risks</button>
    <button class="sugg-btn" onclick="suggest(this)">Upcoming meetings</button>
    <button class="sugg-btn" onclick="suggest(this)">Salesforce pipeline</button>
  </div>
  <div class="inp-row">
...
<script>
  ...
  function togglePanel() {{
    ...
    if (p.classList.contains('open') && !document.getElementById('msgs').firstChild) {{
      addMsg('bob', 'Good morning, '+REP.split(' ')[0]+'. I\u2019m Bob \u2014 your IBM Sales assistant for '+ACCOUNT+'. What do you need today?');
    }}
  }}
  ...
  function addMsg(role, text) {{
    ...
    n.textContent = role === 'bob' ? '\U0001f4bc Bob' : 'You';
    const b = document.createElement('div');
    b.className = 'bubble';
    b.textContent = text;
    w.appendChild(n); w.appendChild(b);
    msgs.appendChild(w);
  ...
  function showTyping() {{
    ...
    t.id = 'typing'; t.className = 'msg msg-bob';
...
_components.html(_fab_html, height=640, scrolling=False)
